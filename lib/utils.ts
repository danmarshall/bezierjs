module BezierJs.utils {
    ...
    export function arcfn(t: number, derivativeFn: Function) {
    ...
    export function between(v: number, m: number, M: number) {
    ...
    export function length(derivativeFn: Function) {
    ...
    export function lerp(r: number, v1: Point, v2: Point) {
        var ret: Point = {
            x: v1.x + r * (v2.x - v1.x),
            y: v1.y + r * (v2.y - v1.y)
        };
    ...
    export function abcratio(t: number, n: number): number {
    ...
    export function projectionratio(t: number, n: number): number {
    ...
    export function lli8(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, x4: number, y4: number): Point {
    ...
    export function lli4(p1: Point, p2: Point, p3: Point, p4: Point) {
        var x1 = p1.x, y1 = p1.y,
            x2 = p2.x, y2 = p2.y,
            x3 = p3.x, y3 = p3.y,
            x4 = p4.x, y4 = p4.y;
    ...
    export function lli(v1: Offset, v2: Offset) {
        return utils.lli4(v1, v1.c, v2, v2.c);
    }

    export function makeline(p1: Point, p2: Point) {
    ...
    export function findbbox(sections: Bezier[]): BBox {
        ...
        sections.forEach(function (s: Bezier) {
            var bbox = s.bbox();
    ...
    export function getminmax(curve: Bezier, d: string, list: number[]): MinMax {
    ...
    export function droots(p: number[]) {
    ...
    export function inflections(points: Point[]) {
    ...
    export function bboxoverlap(b1: BBox, b2: BBox) {
        var dims = ['x', 'y'], len = dims.length, i: number, dim: string, l: number, t: number, d: number
        for (i = 0; i < len; i++) {
            dim = dims[i];
            l = b1[dim].mid;
            t = b2[dim].mid;
    ...
    export function expandbox(bbox: BBox, _bbox: BBox) {
    ...
    export function getccenter(p1: Point, p2: Point, p3: Point) {
        var dx1 = (p2.x - p1.x),
            dy1 = (p2.y - p1.y),
            dx2 = (p3.x - p2.x),
            dy2 = (p3.y - p2.y);
        ...
        var arc = utils.lli8(mx1, my1, mx1n, my1n, mx2, my2, mx2n, my2n) as Arc,
            r = utils.dist(arc, p1),
